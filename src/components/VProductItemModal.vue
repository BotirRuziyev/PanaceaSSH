    ...
    <div class="modal__item">
      ...
      <div class="info__block-container">
        ...
        <div class="modal__product-container">
          ...
          />
          <button class="pur" @click="buyCustomization">Purchase</button>
...
export default {
  ...
  components: {
    Swiper, SwiperSlide, CustomSelect, NotifyModal
  },
  setup() {
    return {
  ...
  data() {
    return {
      selectedServer: 'Chernarus MOD US'
    }
  },
  props: {
    IsActive: {
  ...
  methods: {
    ...
    async buyCustomization() {
      const toast = useToast();
      try {
        const formData = new FormData();
        formData.append('server_id', this.getServerByName(this.selectedServer));
        const response = await this.axios.post('https://api.panaceadayz.com/v2/services.orderSet', formData);
        ...
        setTimeout(function() {
          window.open(response.data.data.link_to_tickets, '_blank');
        }, 5000);
