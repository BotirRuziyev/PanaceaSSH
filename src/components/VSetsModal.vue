<template>
  <div class="modalBg blur" :class="{'modalBg-active': IsOpen === true}">
    <div class="priority__modal">
      ...
      <h1 class="priority__title">Subscribing to Sets</h1>
      <p class="priority__descript">Buy a subscription only if you have a set, or you know how it works" to "Buy a subscription ONLY if you have a set, OR if you know how it works. </p>
      <h2 class="form__title">Enter details</h2>
      <form>
        <div class="select">
          <CustomSelect
              :options="['7 days ($3)', '30 days ($5)', 'Lifetime ($35)']"
              :default="'7 days ($5)'"
              class="select-item"
...
export default {
  ...
  data(){
    return {
      selectedDuration: "7 days ($3)", // Default values
  ...
  methods: {
    ...
    async buy() {
      ...
      try {
        ...
        const response = await this.axios.post('https://api.panaceadayz.com/v2/services.buySubSet', formData);
